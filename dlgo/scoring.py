...
class Territory:
    '''Splits the board into stones, territory and neutral points.'''
...
def evaluate_territory(board):
    ...
    for r in range(1, board.num_rows + 1):
        for c in range(1, board.num_cols + 1):
            p = Point(row=r, col=c)
            if p in status:  # part of different group
                continue
            stone = board.get(p)
            if stone is not None:  # point is a stone, add it as status
                status[p] = board.get(p)
            else:
                group, neighbors = _collect_region(p, board)
                # point is completely surrounded by black or white stones, count it as territory
                if len(neighbors) == 1:
                    neighbor_stone = neighbors.pop()
                    stone_str = 'b' if neighbor_stone == Player.black else 'w'
                    fill_with = 'territory_' + stone_str
                else:
                    fill_with = 'dame'  # point has to be a neutral point, so we add it to dame
                for pos in group:
